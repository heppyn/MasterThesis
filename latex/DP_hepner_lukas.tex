...
Druhá část textu se zabývá procedurálním generováním terénu a využitím L-systémů ke generování vegetace. Pozornost je věnována šumům, zaručujícím deterministické chování generátoru.
}
\abstractEN{
...
The second part of the text deals with procedural generation and possible uses of L-systems to generate vegetation. Attention is focused on noises that guarantee the deterministic behavior of the generator.
}
\placeForDeclarationOfAuthenticity{V~Praze}
\declarationOfAuthenticityOption{4} %volba Prohlášení (číslo 1-6)
\keywordsCS{
vykreslovací engine, voxelová grafika, OpenGL, shadery, procedurální generování, L-systémy
}
\keywordsEN{
rendering engine, voxel graphics, OpenGL, shaders, procedural generation, L-systems
}
\website{https://github.com/heppyn/MasterThesis} %volitelná URL práce, objeví se v tiráži - úplně odstraňte, nemáte-li URL práce
...
\begin{introduction}
V posledním desetiletí zažívají velký rozpuk hry s voxelovou grafikou\footnote{Voxel je bod v prostoru, vykreslený jako krychle.}. Tyto hry se často zaměřují na příležitostné hráče, kteří se ke hře vrací, aby farmařili, budovali své příbytky nebo prozkoumávali procedurálně generovaný svět. Ne\-jen žánry, ale i způsob vývoje prošel dramatickým vývojem.
...
Cílem práce je vytvořit engine vykreslující voxelovou grafiku. Pro část výpoč-tu a vykreslování bude využita grafická karta.
...
Vizualizátor dokáže načíst libovolně velkou scénu a každý blok zobrazuje jako jeden pixel. Díky tomu je možné najednou zobrazit vetší herní plochu, než by bylo možné s dostatečným detailem zobrazit ve 3D prostoru enginu.

\section{Měření výkonu}

K měření výkonu byla použita dvě zařízení, jejíchž parametry jsou zobrazeny v tabulce~\ref{tab:hw}.

\begin{table}\centering
\caption[Testovací zařízení]{Testovací zařízení}\label{tab:hw}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Zařízení} & \textbf{CPU}                                                          & \textbf{GPU}          & \textbf{RAM} \\ \hline
Notebook          & \begin{tabular}[c]{@{}l@{}}Intel Core i7-7600U\\ 2.80GHz\end{tabular} & Intel HD Graphics 620 & 16GB DDR4    \\ \hline
Stolní PC         & \begin{tabular}[c]{@{}l@{}}Intel Core i7-4790K\\ 4.00GHz\end{tabular} & GeForce GTX 960       & 16GB DDR3    \\ \hline
\end{tabular}
\end{table}

Výkonnostní testy byly zaměřeny na zjištění maximálního množství vykreslitelných objektů a jejich vliv na obnovovací frekvenci. Velikost scény byla upravována změnou vykreslovací vzdálenosti udávané v chuncích. Pokud je vykreslovací vzdálenost 10 chunků, velikost scény bude 336 x 336 bloků ($336 = 2 \cdot 10 \cdot 16 + 16$).

Tabulka~\ref{tab:avg_time} zobrazuje průměrnou dobu nutnou pro vykreslení jednoho snímku v milisekundách. Lze si všimnou, že PC s výkonnější grafickou kartou dosahuje přibližně trojnásobného zrychlení. Tabulka~\ref{tab:var_time} udává výběrovou směrodatnou odchylku vypočtenou z doby nutné pro vykreslení snímku. Vý\-kon\-něj\-ší HW dokázal udržel stabilnější vykreslovací frekvenci při rozhlížení se po scéně, způsobující změnu vykreslených objektů.

\begin{table}\centering
\caption[Průměrná doba vykreslení snímku]{Průměrná doba vykreslení snímku}\label{tab:avg_time}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Zařízení} & \textbf{\begin{tabular}[c]{@{}l@{}}17 chunků\tablefootnote{Počet objektů na scéně byl 1 049 644 a 5 707 621 pro vzdálenost 17, respektive 40 chunků.}\\ 3 CMS\tablefootnote{Počet kaskád stínů.}\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}17 chunků\\ 5 CSM\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}40 chunků\\ 3 CSM\end{tabular}} \\ \hline
Notebook & 67,65 & 87,01 & 152,77 \\ \hline
Stolní PC & 20,61 & 26,31 & 62,13 \\ \hline
\end{tabular}
\end{table}

\begin{table}\centering
\caption[Směrodatná odchylka vykreslení snímku]{Směrodatná odchylka vykreslení snímku}\label{tab:var_time}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Zařízení} & \textbf{\begin{tabular}[c]{@{}l@{}}17 chunků\\ 3 CMS\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}17 chunků\\ 5 CSM\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}40 chunků\\ 3 CSM\end{tabular}} \\ \hline
Notebook & 43,65 & 45,30 & 62,17 \\ \hline
Stolní PC & 6,75 & 8,61 & 9,85 \\ \hline
\end{tabular}
\end{table}
